...
* 消息投递语义
    * 生产消息
        ...
        * At least once: `request.required.acks=-1`、`message.send.max.retries`、`replication-factor`、`min.insync.replicas`、`unclean.leader.election.enable`
    ...
    * 消费消息
        ...
        * Exactly once: `At least once`、`事务`、`幂等性`(去重)
* 解耦、削峰、异步化
