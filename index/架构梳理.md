...
* 缓存问题
    * 缓存穿透(无效的key): 缓存空对象、布隆过滤器
    * 缓存雪崩
        * 缓存大量集中失效: 缓存失效时间离散化、二级缓存
        * 宕机: Redis Cluster
    * 缓存击穿(热点key失效): 分布式锁、`永远不过期`
...
* Consumer
    ...
    * 提交offset
        ...
        * 提交指定offset: `enable.auto.commit=false`、`commitSync()`、`commitSync()`
* 消息投递语义
    * 生产消息
        * At most once: `request.required.acks=0`
        * At least once: `request.required.acks=-1`、`message.send.max.retries`、`replication-factor`、`min.insync.replicas>=2`、`unclean.leader.election.enable=false`
...
* 新增用户和权限管理
* 日志存储由本地文件迁移至ES
* 增量日志: InheritableThreadLocal、Guava Cache、AtomicInteger
